...
export function TopUpForm({
  ...
  const [state, formAction] = useActionState(
    mode === "edit" ? updateTopUpAction : createTopUpAction,
    initialState
  );
  ...
  return (
    <form action={formAction} className="grid gap-2.5">
      ...
      <fieldset>
        <legend className="block text-xs font-bold text-[#171717]">Sumber Dana</legend>
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          {sourceAccounts.map((sourceAccount) => (
            <label key={sourceAccount.id} className="cursor-pointer">
              <input
                type="radio"
                name="sourceAccountId"
                value={sourceAccount.id}
                defaultChecked={initialValues?.sourceAccountId === sourceAccount.id}
                className="peer sr-only"
                required
              />
              <span className="flex min-h-9 items-center justify-center rounded-[12px] bg-white px-2.5 text-center text-xs font-bold text-[#525252] shadow-sm shadow-black/5 transition peer-checked:bg-[#171717] peer-checked:text-white peer-focus-visible:ring-4 peer-focus-visible:ring-blue-100">
                {sourceAccount.name}
              </span>
            </label>
          ))}
      ...
      <fieldset>
        <legend className="block text-xs font-bold text-[#171717]">Wallet Tujuan</legend>
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          {wallets.map((wallet) => (
